      ...
      for(var i = 0; i < pts.length; i++) {
        ...
        if(!termRef[1].btref[btkey]) {
          ...
          for(var j = 0; j < bts.length; j++) {
            ...
            _tkey = _tkey.join("__");
            bt.tref[_tkey] = [0, 0, eqn_utils.coeffsForRaiseToPwr.getCoeffForAPwrSet(pwr, _pwrs)];
        ...
        if(termRef[1].tref[tkey]) {
          for(var j = 0; j < termRef[1].tref[tkey].bts.length; j++) {
            ...
            eqn_utils.heap.modified(termHeap, p.href, TermMultiply.TermBracket.heapcmp);
          }
