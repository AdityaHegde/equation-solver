  ...
  processForFactorization : function(termHeap, termRef, termsMeta, seperate) {
    if(this.vari) {
      for(var i = 1; i <= this.pwr; i++) {
        ...
        if(termRef[0][key]) {
          termRef[0][key][0] += i;
          Math.heap.modified(termHeap, termRef[0][key], Term.TermBracket.heapcmp);
        }
        else {
          ...
          Math.heap.insert(termHeap, termRef[0][key], Term.TermBracket.heapcmp);
        }
      }
      if(seperate) {
        if(termRef[1].tref[key]) {
          for(var j = 0; j < termRef[1].tref[key].bts.length; j++) {
            ...
            Math.heap.modified(termHeap, p.href, Term.TermBracket.heapcmp);
          }
        }
        else {
